...
const REVIEWER_PERSONA =
  ...
  '你的最终回答必须是【仅一个 JSON 对象】:第一个字符必须是 {,最后一个字符必须是 },中间不得有 JSON 以外的任何文本。' +
  '禁止:任何解释/前言/总结句、Markdown 代码块(```json)、缩进注释、等待确认。直接输出最终 JSON。' +
  '你没有工具,禁止输出任何工具调用(如 <|DSML| tool_calls>)。如果你输出工具调用,你的回答将被丢弃并视为失败。' +
